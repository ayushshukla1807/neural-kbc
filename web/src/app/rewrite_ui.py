...
# We want to replace the `return (...)` block in `export default function NeuralArena()`.
# The return statement starts with `return (`. We can find the last `return (` in the file.
last_return_idx = code.rfind("return (")
end_idx = code.rfind(");") + 2
...
new_ui = """return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans relative overflow-hidden transition-all duration-1000" style={{ backgroundColor: `hsl(${ambientHue}, 20%, 4%)` }}>
      ...
      <AnimatePresence>
        {pendingAnswer !== null && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/80 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="glass-panel-sleek rounded-[2.5rem] p-12 text-center max-w-lg w-full">
              ...
              <div className="flex gap-4 justify-center mb-10">
                <motion.div key={finalCountdown} initial={{ scale: 1.2, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-5xl font-mono text-amber-500">{finalCountdown}</motion.div>
              </div>
      ...
      <motion.div animate={{ x: mousePos.x * 0.5, y: mousePos.y * 0.5 }} transition={{ type: "spring", stiffness: 100, damping: 30 }} className="relative z-10 w-full h-full flex flex-col items-center justify-center">
        ...
        {gameState === "lobby" && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="z-10 flex flex-col items-center max-w-6xl text-center relative w-full px-6">
            ...
            <div className="flex gap-4 w-full max-w-4xl mt-8">
        ...
        {gameState === "onboarding" && (
          <motion.div initial="hidden" animate="visible" variants={containerVariants} className="z-10 w-full max-w-4xl px-6 text-center">
            <motion.h2 variants={itemVariants} className="text-4xl font-medium tracking-tight text-white mb-3 drop-shadow-md">Configure Sandbox</motion.h2>
            ...
            <motion.div variants={containerVariants} className="flex flex-wrap justify-center gap-3 mb-10">
              {availableDomains.map(domain => {
                ...
                return ( <motion.button variants={itemVariants} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} key={domain} onClick={() => toggleDomain(domain)} className={`px-6 py-3 rounded-2xl font-medium tracking-wide transition-all text-sm ${selected ? 'bg-white text-black shadow-[0_0_24px_rgba(255,255,255,0.4)]' : 'bg-surface-100 border border-white/5 text-zinc-400 hover:bg-surface-200'}`}> {domain} </motion.button> )
              })}
            </motion.div>
            
            <motion.button variants={itemVariants} onClick={constructGenerativeMatrix} disabled={selectedDomains.length !== 3} className={`px-10 py-4 rounded-full font-bold tracking-widest text-xs uppercase transition-all ${selectedDomains.length === 3 ? 'bg-amber-500 text-zinc-950 shadow-[0_4px_24px_rgba(245,158,11,0.4)] hover:bg-amber-400' : 'bg-surface-200 text-zinc-600 cursor-not-allowed border border-white/5'}`}>
              Initialize Matrix
            </motion.button>
          </motion.div>
        )}
        ...
        {gameState === "loading_llm" && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="z-10 flex flex-col items-center">
            <div className="spinner-ring mb-8"></div>
        ...
        {gameState === "decrypting" && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/90 backdrop-blur-2xl">
            <div className="flex flex-col items-center">
              <div className="spinner-ring mb-10 w-16 h-16 border-t-amber-500"></div>
        ...
        {/* ENDGAME STATES */}
         {["eliminated", "victorious", "extracted"].includes(gameState) && (
          <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 glass-panel-sleek p-16 rounded-[3rem] max-w-3xl w-full text-center relative overflow-hidden flex flex-col items-center">
            <h1 className="text-5xl font-medium mb-4 tracking-tight relative z-10 text-white">
            ...
            {playMode?.startsWith("duel") ? (
               ...
               <div className="my-10">
                 <p className="text-xs text-zinc-500 font-medium uppercase tracking-[0.3em] mb-3">{playMode === 'interview' ? 'Career Score' : 'Final Ledger'}</p>
                 <p className="text-6xl font-mono text-white tracking-tight drop-shadow-lg">{playMode === 'interview' ? `${interviewXP} XP` : walletBalance}</p>
               </div>
            )}
        ...
        {q && gameState === "active" && (
          <div className="z-10 flex flex-col w-full h-full max-w-7xl mx-auto px-6 py-8 relative">
            ...
            <header className="flex justify-between items-center w-full pb-8">
              <div className="flex items-center justify-center gap-12">
                  <div className="flex flex-col">
                    <span className="text-zinc-500 tracking-[0.2em] text-[9px] uppercase font-bold mb-1">{playMode === 'interview' ? 'Quota' : 'Ledger'}</span>
                    <span className="font-mono text-white tracking-wider text-xl">{playMode === 'interview' ? `${interviewXP} XP` : walletBalance}</span>
                  </div>

                  <div className="flex flex-col items-center">
                     <span className="text-zinc-500 tracking-[0.2em] text-[9px] uppercase font-bold mb-1">Level</span>
                     <span className="font-mono text-white tracking-widest text-lg">{String(level+1).padStart(2,'0')} <span className="text-zinc-600">/ 16</span></span>
                  </div>
                  ...
                  {playMode === "solo" && streak >= 2 && (
                    <motion.div initial={{scale:0.8,opacity:0}} animate={{scale:1,opacity:1}} className="px-4 py-1.5 rounded-full border border-amber-500/20 bg-amber-500/10 flex items-center gap-2 ml-4">
              ...
              <div className="flex items-center gap-4">
                ...
                {playMode === "solo" && (
                  <button onClick={() => { VoiceEngine.speak("Leaving already? Stay safe."); setGameState("extracted"); }} className="px-6 py-2 rounded-full text-[10px] font-bold tracking-widest uppercase text-zinc-400 hover:text-white border border-transparent hover:border-white/10 transition-colors">Withdraw</button>
                )}
              </div>
            </header>
            ...
            <div className="flex-1 flex gap-12 w-full">
              {/* Left Column: Money Ladder (only solo) */}
              {playMode === "solo" && (
                 <div className="hidden lg:flex w-[260px] pt-12 items-start opacity-70">
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                ...
                <div className="w-full max-w-[800px] mb-12 flex flex-col items-center gap-3">
                   <span className={`text-4xl font-mono font-medium tracking-tight ${timer <= 15 ? 'text-amber-500' : 'text-zinc-300'}`}>
                     {timer.toString().padStart(2,'0')}
                   </span>
                   <div className="w-full h-[3px] bg-surface-200 rounded-full overflow-hidden">
                     <div className={`h-full ${timer <= 15 ? 'bg-amber-500' : 'bg-zinc-400'} timer-bar-progress`} style={{ width: `${(timer/90)*100}%` }} />
                   </div>
                </div>

                <AnimatePresence mode="wait">
                  <motion.div key={`block-${level}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="w-full flex flex-col items-center max-w-[840px]">
                    
                    <div className="w-full min-h-[160px] flex flex-col justify-center items-center text-center px-4 mb-8">
                       <motion.h2 key={`q-${level}`} className="text-3xl md:text-[2.5rem] font-medium leading-[1.3] text-white tracking-tight">
                         {displayedQuestion}<span className="animate-pulse text-zinc-500 ml-1">|</span>
                       </motion.h2>

                       {/* Audience Poll Chart */}
                       {showPoll && (
                         <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="mt-10 p-5 bg-surface-100 border border-white/5 rounded-2xl w-full max-w-sm flex items-end justify-between h-28 gap-4">
                           {['A','B','C','D'].map((ltr, i) => (
                             <div key={ltr} className="flex flex-col items-center flex-1 gap-2 h-full justify-end">
                               <span className="text-[10px] text-zinc-500 font-mono">{showPoll[i]}%</span>
                               <div className="w-full bg-white/5 rounded-t relative overflow-hidden" style={{height:`${showPoll[i] * 0.9}%`, minHeight:'4px'}}>
                                 <div className="absolute inset-0 bg-white opacity-20 poll-bar" style={{'--target-w':'100%'} as any} />
                               </div>
                       ...
                       {oracleLog && !showPoll && (
                         <motion.div initial={{opacity:0}} animate={{opacity:1}} className="mt-8 text-sm font-light text-zinc-400 bg-surface-100 px-6 py-3 rounded-xl border border-white/5">
                           {oracleLog}
                         </motion.div>
                       )}
                    </div>
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 relative z-20">
                      {q.options.map((opt: string, i: number) => (
                         <div key={`opt-${level}-${i}`} className="relative pl-0">
                           {hoveredOpt === i && selectedOpt === null && pendingAnswer === null && aiHints.current[i] !== undefined && (
                             <motion.div initial={{opacity:0, y:2}} animate={{opacity:1, y:0}} className="absolute -top-7 left-1/2 -translate-x-1/2 z-30 px-3 py-1 rounded bg-zinc-800 text-[10px] text-zinc-400 whitespace-nowrap border border-white/5 shadow-xl">
                               <span className="text-amber-500 mr-2">✦</span> {aiHints.current[i]}% Probability
                             </motion.div>
                           )}
               ...
               {playMode === "solo" && <div className="hidden lg:block w-[260px]" />}
            </div>
          </div>
        )}
      </motion.div>
...
new_code = code[:last_return_idx] + new_ui + "\n}\n"
